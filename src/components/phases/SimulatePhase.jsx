...
import AppleOrchardStation from '../simulations/AppleOrchardStation.jsx';
import BalanceScaleStation from '../simulations/BalanceScaleStation.jsx';
import InverseMachineStation from '../simulations/InverseMachineStation.jsx';
import FactDetectiveStation from '../simulations/FactDetectiveStation.jsx';
import { useAudio } from '../../hooks/useAudio.js';
import { simStationIntro } from '../../utils/narration.js';

const STATIONS = [
  { id: 0, label: 'A', name: 'Apple Orchard',     icon: '🍎', desc: 'Concrete manipulatives' },
  { id: 1, label: 'B', name: 'Balance Scale',     icon: '⚖️', desc: 'Scale balance & triangle' },
  { id: 2, label: 'C', name: 'Inverse Machine',   icon: '⚙️', desc: 'Operator conversion gizmo' },
  { id: 3, label: 'D', name: 'Fact Detective',    icon: '🔍', desc: 'Error buster case files' },
];
    ...
    <div className="sim-wrap">
      <div className="sim-card glass-card">
        ...
        <div className="sim-station-area" role="tabpanel" key={s}>
          {s === 0 && <AppleOrchardStation onComplete={() => handleStationComplete(0)} audioEnabled={state?.audioEnabled} />}
          {s === 1 && <BalanceScaleStation onComplete={() => handleStationComplete(1)} audioEnabled={state?.audioEnabled} />}
          {s === 2 && <InverseMachineStation onComplete={() => handleStationComplete(2)} audioEnabled={state?.audioEnabled} />}
          {s === 3 && <FactDetectiveStation onComplete={() => handleStationComplete(3)} audioEnabled={state?.audioEnabled} />}
        </div>
